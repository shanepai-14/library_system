...
const StudentAccount = () => {
  ...
  const editableColumns = ["first_name","last_name","middle_name","address","contact_number"];  
  const handleEdit = () => {
    setEdit(true);
  }
  const handleClose = () => {
  ...
  <Typography variant="h4" align="center" sx={{marginBottom:5,marginTop:2}}>{userData.role =="admin" ? "Account ": "Student"} Details</Typography>
      <Grid display={'flex'} justifyContent={'center'} flexDirection={'row'} alignItems={"center"} container spacing={2} direction="row">

        <Grid item container  spacing={3} lg={8}>
          <Grid  pl={0} pt={0}  xs={12} sm={4} md={3}  justifyContent={"center"} alignItems={"center"} display={'flex'}>
            <Avatar
